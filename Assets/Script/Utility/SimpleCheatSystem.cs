...
public class SimpleCheatSystem : MonoBehaviour
{
    ...
    private void InitiallnezeCommands()
    {
    ...
    private void CloearConsole(string[] args)
    {
        outputLines.Clear();
        if (outputText != null)
            outputText.text = "";
    ...
    private void ShowToast(string message, ToastMessage.MessageType type = ToastMessage.MessageType.info)
    {
        ...
        {
            ToastMessage.instance.SendMessage($"[치트] {message}", type);
        }
    }
